 ...
 *
 * Для плавной анимации модуль вызывается на каждом кадре. В автономном режиме
 * те же формулы использует localSimulator.ts на расчётной сетке 120 с.
 *
 * В remote-режиме константы можно взять из ответа /simulate, чтобы фронт
 * и API считали одинаково и не разъезжались.
 */
