"""
УСТАРЕЛО: маршрутизация переехала в services/routing.py, этот модуль нигде
не используется. Оставлен как справочный вместе со своими тестами.

Почему переписали, а не починили: поиск шёл от шлюзов, путь восстанавливался
перебором соседей и на неоднозначности бросал ValueError, клиенты без единого
ребра не попадали в ответ вовсе, а проверка `vertex == VertexType.client`
сравнивала строку с элементом перечисления и всегда была ложной — из-за чего
трафик мог ретранслироваться через другой наземный пункт.
"""
